...
function vk_bot_parse_mod_command($userMsg, $msgObj)
{
    ...
    } elseif (preg_match('/^(?:[!|\/](?:справка|помощь|команды|commands)|!справка\b|!помощь\b|справка\b)/ui', $clean)) {
        $type = 'user_help';
    }
...
function vk_bot_get_admin_help_text($vkGroupId = 0)
{
    $text = "🛠️ СПРАВОЧНИК ВСЕХ КОМАНД РОБОТА КОСМО ДЛЯ АДМИНИСТРАТОРОВ 🤖\n"
          ...
          . "• Где библиотеки / Адреса библиотек / Филиалы — адреса, телефоны и районы всех 18 филиалов.\n"
