...
| **`до`** | `ЦГБ-ДО` | Центральная городская библиотека, Детский отдел | Суздальский пр., д. 2 | **жилой район «Доброе»** | 8(4922) 21-65-63 | **ДА** | Нет |
...
3. **Все библиотеки района «Доброе»:**  
   ...
   - ЦГБ (Суздальский пр., д. 2): Абонемент (`аб`), Читальный зал (`чз`), Детский отдел (`до`), Книгохранилище (`кх`), ИБО (`ибо`), Отдел обслуживания (`ооо`);
