...
| **`до`** | `ЦГБ-ДО` | Центральная детская библиотека (алиас «ДО») | ул. Большая Московская, д. 31 | **Исторический центр** | 8(4922) 32-32-42, 32-47-73 | Нет | **ДА (ЦДБ)** |
| **`цдб`** | `ЦГБ-ЦДБ`, `цди`, `цки` | Центральная детская библиотека | ул. Большая Московская, д. 31 | **Исторический центр** | 8(4922) 32-32-42, 32-47-73 | Нет | **ДА (ЕДИНСТВЕННАЯ!)** |
| **`цдч`** | `ЦГБ-ЦДЧ` | Центральная городская библиотека, Отдел детского чтения | Суздальский пр., д. 2 | **жилой район «Доброе»** | 8(4922) 21-65-63, 21-66-80 | **ДА** | Нет |
...
def normalize_sigla(raw_sigla: str) -> str:
    ...
    if s in ['до', 'цдб', 'цди', 'цки']: return 'цдб'
    if s in ['цдч']: return 'цдч'
