...
import { EARTH_CONFIG, EARTH_CENTER } from './iss_station.js?v=4.18.2';
...
export class SatellitesSwarmEngine {
    ...
    /**
     * Генерация 10 спутников со строго непересекающимися орбитальными оболочками
     */
    generateSwarm() {
        this.satellites = [];
        const count = 10;
        ...
        // Шаг разделения: 16.0 px между соседними сферическими оболочками
        // Скорость уменьшена ровно в 2 раза: 144 секунды на виток для низкой орбиты вместо 72 с!
        const baseRadius = 790;
        const deltaRadius = 16.0;
        const baseOmega = (2 * Math.PI) / 144.0;
        ...
        for (let i = 0; i < count; i++) {
            ...
            this.satellites.push({
                ...
                incDeg,
                raanDeg,
                // Лёгкая прецессия RAAN (медленный дрейф узла, 0.01-0.03 °/с, знакопеременный)
                raanDriftDegPerSec: ((i % 2 === 0 ? 1 : -1) * (0.02 + ((i * 7) % 5) * 0.01)) * 0.5,
                phase: initialPhase,
                pNode,
                qNode,
                altKm,
